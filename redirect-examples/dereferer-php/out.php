...
$config = array();
// path to basename(__FILE__) (this script). Leave emtpy for default setups.
$config['hostnameAndPath'] = '';
// Default redirect URL. Used in case we have Problems resetting the referer.
$config['redirectToIfWrongReferer'] = 'error.refererNotChanged.html';
...
// Redirect Sequence Part 1 - This is where we end up in our first redirect loop in basename(__FILE__)
if(!isset($_REQUEST['go'])) { 
    
    $redirect_link_form  = $config['hostnameAndPath'].basename(__FILE__); // redirect to go first
...
// Redirect Sequence Part 2 - second redirect to target link    
    ...
    if (isset($_SERVER['HTTP_REFERER']) && strlen($_SERVER['HTTP_REFERER']) && stripos( $_SERVER['HTTP_REFERER'], $config['hostnameAndPath'].basename(__FILE__) ) === false )
    {
...
<html xmlns="http://www.w3.org/1999/xhtml" dir="ltr" lang="de">
    ...
    <body>
        ...
        <div style="padding: 30px; text-align: center;">
            <a href="<?php echo $redirect_link; ?>">Redirect Message Lorem Ipsum Blub - click here if you have no js</a>
        </div>
